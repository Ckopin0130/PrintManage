      ...
      <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl relative mb-10" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-5 mb-6">
           ...
           <div>
             ...
             {!useCustomModel ? (
               <div className="flex gap-2">
                 <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none text-slate-800 font-bold text-lg" 
                ...
                <div className="space-y-3 animate-in slide-in-from-top-2">
                  <div className="flex gap-2">
                    <input autoFocus placeholder="輸入新分類名稱" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none font-bold text-lg" value={formData.model} onChange={e => setFormData({...formData, model: e.target.value})} />
                      ...
                      <div className="flex flex-wrap gap-2">
                          {Object.keys(BIG_CATEGORY_CONFIG).map(key => (
                              <button 
                                ...
                                className={`px-3 py-2 rounded-lg text-sm font-bold border transition-all ${formData.categoryType === key ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white text-slate-500 border-slate-200'}`}
                              >
           ...
           <div>
               ...
               <input placeholder="例: 黃色碳粉" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none text-xl text-slate-800 font-bold placeholder:font-normal" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
           </div>

           <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100">
               <label className="text-xs font-bold text-blue-500 block mb-1.5 uppercase tracking-wider flex items-center">
                   <Tag size={14} className="mr-1"/> 次分類 (選填)
               </label>
               <input 
                  placeholder="例如: C3503 (相同名稱會自動分組)" 
                  className="w-full bg-white border border-blue-200 rounded-lg py-2 px-3 outline-none text-lg text-slate-800 font-bold placeholder:font-normal placeholder:text-slate-400 focus:ring-2 focus:ring-blue-100" 
           ...
           <div className="grid grid-cols-3 gap-3">
              <div className="col-span-1">
                  ...
                  <input type="number" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none text-center font-mono font-bold text-2xl text-blue-600" value={formData.qty} onChange={e => setFormData({...formData, qty: Number(e.target.value)})} />
              </div>
              <div className="col-span-1">
                  <label className="text-xs font-bold text-slate-400 block mb-1.5 text-center">應備</label>
                  <input type="number" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none text-center font-mono font-bold text-xl" value={formData.max} onChange={e => setFormData({...formData, max: Number(e.target.value)})} />
              </div>
              <div className="col-span-1">
                  <label className="text-xs font-bold text-slate-400 block mb-1.5 text-center">單位</label>
                  <input placeholder="個" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 outline-none text-center font-bold text-xl" value={formData.unit} onChange={e => setFormData({...formData, unit: e.target.value})} />
              </div>
           </div>
        </div>
        <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 py-4 bg-slate-100 font-bold text-slate-500 rounded-xl hover:bg-slate-200 transition-colors text-lg">取消</button>
            <button onClick={() => { if(formData.name && formData.model) onSave(formData); }} className="flex-1 py-4 bg-blue-600 font-bold text-white rounded-xl shadow-lg shadow-blue-200 hover:bg-blue-700 transition-colors active:scale-95 text-lg">儲存</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[80] flex items-start justify-center pt-24 p-4 animate-in fade-in" onClick={onClose}>
      <div className="bg-white w-full max-w-xs rounded-2xl p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <input autoFocus className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 outline-none mb-6 font-bold text-xl text-slate-700" value={newName} onChange={e => setNewName(e.target.value)} />
        <div className="flex gap-2">
...
const SortableBigCategory = ({ category, count, onClick, onEditLabel }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} onClick={onClick} className="w-full bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center active:scale-[0.98] transition-all hover:border-blue-200 group mb-3 relative touch-manipulation cursor-pointer">
            <div className={`p-4 rounded-xl mr-4 border transition-colors shadow-sm ${category.color}`}>
                <Icon size={28} strokeWidth={2.5} />
            </div>
            <div className="flex-1 text-left min-w-0">
                <h3 className="text-xl font-extrabold text-slate-800 truncate mb-1">{category.label}</h3>
                <span className="text-sm text-slate-500 font-bold">共 {count} 個項目</span>
            </div>
            <div className="flex items-center gap-1 ml-2">
                <button onClick={(e) => { e.stopPropagation(); onEditLabel(category.id, category.label); }} className="p-3 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-colors"><Edit3 size={20} /></button>
                <div {...attributes} {...listeners} className="text-slate-300 cursor-grab active:cursor-grabbing hover:text-slate-500 p-3" onClick={(e) => e.stopPropagation()}><GripVertical size={24} /></div>
            </div>
        </div>
    );
};
...
const InventoryRow = ({ item, onEdit, onRestock, isLast }) => {
    ...
    return (
        <div className={`flex items-center justify-between py-4 px-4 transition-colors ${rowClass} ${borderClass}`}>
            <div className="flex items-center flex-1 min-w-0 mr-3 cursor-pointer" onClick={() => onEdit(item)}>
                <div className="flex flex-col">
                    <span className={`text-lg font-bold truncate leading-tight ${textClass}`}>{item.name}</span>
                    <span className="text-xs text-slate-400 mt-1 font-bold">{item.unit}</span>
                </div>
                {isOut && <span className="ml-3 px-2 py-1 bg-rose-200 text-rose-700 text-xs font-black rounded shrink-0">缺貨</span>}
            </div>
            
            <div className="flex items-center gap-4 shrink-0">
                <div className={`font-mono font-bold text-xl ${isOut ? 'text-rose-600' : 'text-blue-600'}`}>
                    {item.qty} <span className="text-slate-300 text-sm font-bold">/ {item.max}</span>
                </div>
                {item.qty < item.max ? (
                    <button onClick={() => onRestock(item.id, item.max)} className="p-2 rounded-xl bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition-colors shadow-sm active:scale-90">
                        <RotateCcw size={20} />
                    </button>
                ) : ( <div className="p-2 text-emerald-400"><CheckCircle size={22} /></div> )}
            </div>
        </div>
    );
}
...
const AccordionGroup = ({ groupName, items, onEdit, onRestock }) => {
    ...
    return (
        <div className="border border-slate-200 bg-white rounded-2xl overflow-hidden shadow-sm mb-3">
            <div onClick={() => setIsOpen(!isOpen)} className={`flex justify-between items-center px-4 py-3.5 cursor-pointer select-none transition-colors ${isOpen ? 'bg-slate-50 border-b border-slate-100' : 'bg-white hover:bg-slate-50'}`}>
                <div className="flex items-center gap-2">
                    {isOpen ? <ChevronDown size={20} className="text-slate-400"/> : <ChevronRight size={20} className="text-slate-400"/>}
                    <span className="text-lg font-extrabold text-slate-800">{groupName}</span>
                </div>
                <div className="flex items-center gap-3">
                    {lowStockCount > 0 && <span className="flex items-center text-xs font-bold text-rose-500 bg-rose-50 px-2 py-1 rounded-full"><AlertTriangle size={12} className="mr-1"/> {lowStockCount} 缺</span>}
                    <span className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-md">{items.length} 項</span>
                </div>
            </div>
            {isOpen && (
...
const ModelListRow = ({ title, count, lowStock, onClick, onRename, categoryType }) => {
    ...
    return (
        <div onClick={onClick} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-[0_1px_3px_rgb(0,0,0,0.02)] active:scale-[0.99] transition-all cursor-pointer flex items-center justify-between mb-3 hover:border-blue-200 hover:shadow-md group">
            <div className="flex items-center flex-1 min-w-0">
                <div className={`p-3 rounded-xl mr-4 shrink-0 bg-slate-50 text-slate-500`}>
                    <Icon size={24} />
                </div>
                <div className="min-w-0">
                    <h3 className="text-lg font-extrabold text-slate-800 truncate mb-1">{title}</h3>
                    <div className="flex items-center gap-2 text-sm font-bold text-slate-400">
                        <span>{count} 個項目</span>
                        {lowStock > 0 && <span className="text-rose-500 flex items-center bg-rose-50 px-1.5 py-0.5 rounded"><AlertTriangle size={12} className="mr-1"/> {lowStock} 缺</span>}
                    </div>
                </div>
            </div>
            
            <div className="flex items-center pl-2">
                <button onClick={(e) => { e.stopPropagation(); onRename(title); }} className="p-3 text-slate-300 hover:text-blue-500 hover:bg-slate-50 rounded-xl transition-colors mr-1">
                    <Edit3 size={20} />
                </button>
                <ChevronRight className="text-slate-300 group-hover:text-blue-400 transition-colors" size={24} />
            </div>
        </div>
    );
};
...
const InventoryView = ({ inventory, onUpdateInventory, onAddInventory, onDeleteInventory, onRenameGroup, onBack }) => {
  ...
  const [bigGroupLabels, setBigGroupLabels] = useState(() => {
    try {
        const saved = localStorage.getItem('inventoryBigLabels');
        return saved ? JSON.parse(saved) : DEFAULT_BIG_LABELS;
    } catch { return DEFAULT_BIG_LABELS; }
  });
  ...
  const [categoryOrder, setCategoryOrder] = useState(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : DEFAULT_CATEGORY_ORDER;
  ...
  const handleDeleteBigGroup = (id) => {
      if (!window.confirm(`確定要隱藏「${bigGroupLabels[id]}」分類嗎？(您可以隨時重置恢復)`)) return;
       ...
       <div className="bg-white/95 backdrop-blur px-4 py-3 shadow-sm sticky top-0 z-30 border-b border-slate-100/50">
         <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-2">
                {!selectedBigGroup && (
                    <>
                    <button onClick={handleResetLayout} className="p-2 bg-slate-100 text-slate-400 rounded-xl hover:bg-slate-200 hover:text-slate-600 transition-colors" title="重置版面">
                        <RefreshCw size={20}/>
                    </button>
                    <button onClick={() => setShowReport(true)} className="p-2 bg-slate-100 text-slate-600 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-colors" title="產生報表">
                        <FileText size={20}/>
                    </button>
                    </>
                )}
                {/* 排序按鈕：現在同時支援 Level 2 (selectedBigGroup) 與 Level 3 (activeCategory) */}
                {(selectedBigGroup || activeCategory) && (
                    <button onClick={() => setSortMode(prev => prev === 'name' ? 'qty' : 'name')} className="p-2 px-3 bg-slate-100 text-slate-600 rounded-xl font-bold text-sm flex items-center shadow-sm active:scale-95 transition-transform">
                        {sortMode === 'name' ? <SortAsc size={18} className="mr-1.5"/> : <SortDesc size={18} className="mr-1.5"/>}
                        {sortMode === 'name' ? '名稱' : '數量'}
                    </button>
                )}
